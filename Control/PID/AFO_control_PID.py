...
class walker_control_node(Node):
    def __init__(self):
        ...
        self.ts = message_filters.ApproximateTimeSynchronizer([scan_sub, encoder_sub], queue_size=20, slop=0.10)
    ...
    def motor_publish_callback(self):
        ...
        if stale:
            self.get_logger().warning("Motor command zeroed: " f"scan_age={scan_age:.3f}s, " f"encoder_age={encoder_age:.3f}s", throttle_duration_sec=0.5)
    ...
    def control_loop_callback(self):
        ...
        try:
            encoder_velocity = (self.encoder.velocity[0] + self.encoder.velocity[1]) / 2.0
            ...
            step_result = self.main.step_from_legs(current_time, encoder_velocity, raw_left[0], raw_right[0], isoccluded)

        except Exception as e:
            self.latest_wheel_velocity = 0.0
            self.get_logger().error(f"control_loop_callback failed; command zeroed: {e}", throttle_duration_sec=1.0)
            return
...
def main(args=None):
    ...
    try:
        ...
        if hasattr(walker_node, "main") and len(walker_node.main.commanded_timestamps) > 0:
            ...
            start_idx = len(walker_node.main.encoder_data) - len(walker_node.main.commanded_velocity_history)
            ...
            print(f"Mean Command: {np.nanmean(walker_node.main.commanded_velocity_history):.3f} rad/s")
            ...
            axs[2, 0].plot(walker_node.main.commanded_timestamps, walker_node.main.stride_used_history, label="Stride Used")
